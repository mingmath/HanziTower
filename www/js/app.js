...
const Modal = {
    openBase(title, content, icon) {
        ...
        document.getElementById('modal-content').className = 'modal-body'; // 重置特效
    ...
    showVictory(title, msg, actionsHtml) {
        ...
        document.getElementById('modal-content').className = 'modal-body modal-pop';
        document.getElementById('modal').style.display = 'flex';
    },
    // 升級視窗 (帶金光特效)
    showLevelUp(title, msg) {
        ...
        document.getElementById('modal-content').className = 'modal-body modal-levelup';
        document.getElementById('modal').style.display = 'flex';
    },
...
const App = {
    ...
    ensureRealmLoaded(realmIdx, callback) {
        ...
        script.onerror = () => {
            this.isLoadingRealm = false;
            Modal.show("錯誤", "無法讀取關卡資料", "重試", "⚠️", () => this.ensureRealmLoaded(realmIdx, callback));
        };
    ...
    bindEvents() {
        ...
        bind('btn-hint', () => Game.useHint()); // 舊的，保留相容
    ...
    showMissions() {
        ...
        targets.forEach((target, idx) => {
            ...
            let btnHtml = '';
            if (claimed) {
                btnHtml = `<button class="btn-claim" disabled>已領取</button>`;
            } else if (achieved) {
                btnHtml = `<button class="btn-claim" onclick="App.claimMission(${idx})">領取</button>`;
            } else {
                btnHtml = `<span class="text-xs text-stone-400">${Data.mission.count}/${target}</span>`;
            }

            div.innerHTML = `
                <div>
                    <div class="mission-desc">完成 ${target} 個新關卡</div>
                    <div class="mission-reward">🪙${rewards[idx].coins}  🏅${rewards[idx].xp}XP</div>
                </div>
                ${btnHtml}
            `;
    ...
    checkDailyStatus() {
        ...
        if(btnHome) { 
            btnHome.innerHTML = isChecked ? "📅 今日已簽到" : "📅 簽到領蠟燭"; 
            btnHome.style.opacity = isChecked ? "0.5":"1"; 
        }
...
const Game = {
    ...
    _startLevelInternal(idx) {
        ...
        if (!lvl) { 
            Modal.show("錯誤", "無資料", "返回", "⚠️", () => App.showLevels()); 
            return; 
        }
        ...
        if(hintArea) { hintArea.innerHTML = ''; hintArea.className = ''; }
        ...
        if (idx === 0) { 
            setTimeout(() => { NPC.say("歡迎來到漢字的國度！...", "好", () => { this.tutorialStep = 1; this.updateTutorialUI(); }); }, 500); 
        } 
        else if (idx === 1 && Data.keys < 100) { 
            setTimeout(() => { NPC.say("卡關了怎麼辦？", "怎麼辦", () => { NPC.say("這回合免費使用提示！", "試試看", () => { this.tutorialStep = 30; this.updateTutorialUI(); }); }); }, 500); 
        }
    },
    ...
    scriptedHint(word) {
        ...
        if(area) { 
            ...
            this.hintTimeout = setTimeout(() => { area.classList.remove('show'); }, 4000); 
        }
    },
    ...
    checkAnswer() {
        ...
        if(player.sort().join(',') === [...lvl.sols].sort().join(',')) {
            ...
            if(this.currentLevelIdx >= Data.unlockedLevel) {
                ...
                Data.coins += 1; // 舊關卡僅加金幣
            }
            Data.save();
            App.updateUI();

            ConfettiSystem.spawn(); // 撒花特效
            ...
            this.showVictory = showVictory; // 綁定給 showVocab 呼叫
            this.showVocab = () => {
